...
--
-- Host: localhost    Database: auto_billing_dw
-- ------------------------------------------------------
...
--
/*!50003 DROP FUNCTION IF EXISTS `calc_p2pe_encryption_charge` */;
...
DELIMITER ;;
CREATE DEFINER=`data_warehouse`@`172.16.63.%` FUNCTION `calc_p2pe_encryption_charge`(
     f_p2pe_auth_trans          DECIMAL(16, 4)
    ,f_p2pe_refund_trans        DECIMAL(16, 4)
    ,f_p2pe_credit_trans        DECIMAL(16, 4)
    ,f_p2pe_auth_decline_trans  DECIMAL(16, 4)
    ,f_p2pe_encryption_fee      DECIMAL(16, 4)
    ,f_decryption_count         DECIMAL(16, 4)
) RETURNS decimal(16,4)
BEGIN

  DECLARE return_val  DECIMAL(16, 4);

-- IF  f_p2pe_auth_trans IS NULL OR f_p2pe_refund_trans IS NULL OR f_p2pe_credit_trans IS NULL OR f_p2pe_auth_decline_trans IS NULL THEN 
--     SET return_val = f_p2pe_encryption_fee * f_decryptions_mdt;
-- ELSE (p2pe_auth_trans + p2pe_refund_trans + p2pe_credit_trans + p2pe_auth_decline_trans) > 0 THEN 
--    p2pe_encryption_charge = p2pe_encryption_fee * (p2pe_auth_trans + p2pe_refund_trans + p2pe_credit_trans + p2pe_auth_decline_trans)
-- ELSE 
--    p2pe_encryption_charge = p2pe_encryption_fee * decryptions_mdt


/*
account_id        |p2pe_auth_trans|p2pe_refund_trans|p2pe_credit_trans|p2pe_auth_decline_trans|pricing_p2pe_encryption_fee|decryption_count|p2pe_encryption_charge|
------------------|---------------|-----------------|-----------------|-----------------------|---------------------------|----------------|----------------------|
0013i00000Cg8RaAAJ|               |                 |                 |                       |                     0.1000|          2.0000|                0.2000|
0013i00000FhBuoAAF|        91.0000|           0.0000|           0.0000|                12.0000|                     0.0500|                |                5.1500|
0013i00000FhBvIAAV|         0.0000|           0.0000|           0.0000|                 0.0000|                     0.1000|         98.0000|                0.0000|
*/
    IF f_p2pe_auth_trans IS NULL OR f_p2pe_refund_trans IS NULL OR f_p2pe_credit_trans IS NULL OR f_p2pe_auth_decline_trans IS NULL THEN 
        SET return_val = f_p2pe_encryption_fee * f_decryption_count;
    ELSEIF f_p2pe_auth_trans + f_p2pe_refund_trans + f_p2pe_credit_trans + f_p2pe_auth_decline_trans > 0 THEN 
        SET return_val = f_p2pe_encryption_fee * (f_p2pe_auth_trans + f_p2pe_refund_trans + f_p2pe_credit_trans + f_p2pe_auth_decline_trans);
    ELSE 
        SET return_val = f_p2pe_encryption_fee * f_decryption_count;
    END IF;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `test_auto_billing_complete_2` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES' */ ;
DELIMITER ;;
CREATE DEFINER=`data_warehouse`@`172.16.63.%` PROCEDURE `test_auto_billing_complete_2`()
BEGIN
  
  SELECT 'creating test.tmp_abc_err' AS message;

  DROP TABLE IF EXISTS test.tmp_abc_err;

  CREATE TEMPORARY TABLE test.tmp_abc_err
        SELECT abc1.cardconex_acct_id, 'achworks_credit_charge                   ' AS column_name, abc1.achworks_credit_charge                   AS abc_1           , abc2.achworks_credit_charge                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.achworks_credit_charge                  , 0) != COALESCE(abc2.achworks_credit_charge                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'achworks_monthly_charge                  ' AS column_name, abc1.achworks_monthly_charge                  AS abc_1           , abc2.achworks_monthly_charge                  AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.achworks_monthly_charge                 , 0) != COALESCE(abc2.achworks_monthly_charge                 , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'achworks_per_trans_charge                ' AS column_name, abc1.achworks_per_trans_charge                AS abc_1           , abc2.achworks_per_trans_charge                AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.achworks_per_trans_charge               , 0) != COALESCE(abc2.achworks_per_trans_charge               , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_batches                              ' AS column_name, abc1.ach_batches                              AS abc_1           , abc2.ach_batches                              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_batches                             , 0) != COALESCE(abc2.ach_batches                             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_credit_trans                         ' AS column_name, abc1.ach_credit_trans                         AS abc_1           , abc2.ach_credit_trans                         AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_credit_trans                        , 0) != COALESCE(abc2.ach_credit_trans                        , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_credit_vol                           ' AS column_name, abc1.ach_credit_vol                           AS abc_1           , abc2.ach_credit_vol                           AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_credit_vol                          , 0) != COALESCE(abc2.ach_credit_vol                          , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_errors                               ' AS column_name, abc1.ach_errors                               AS abc_1           , abc2.ach_errors                               AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_errors                              , 0) != COALESCE(abc2.ach_errors                              , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_noc_messages                         ' AS column_name, abc1.ach_noc_messages                         AS abc_1           , abc2.ach_noc_messages                         AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_noc_messages                        , 0) != COALESCE(abc2.ach_noc_messages                        , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_noc_message_charge                   ' AS column_name, abc1.ach_noc_message_charge                   AS abc_1           , abc2.ach_noc_message_charge                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_noc_message_charge                  , 0) != COALESCE(abc2.ach_noc_message_charge                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_returnerror_charge                   ' AS column_name, abc1.ach_returnerror_charge                   AS abc_1           , abc2.ach_returnerror_charge                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_returnerror_charge                  , 0) != COALESCE(abc2.ach_returnerror_charge                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_returns                              ' AS column_name, abc1.ach_returns                              AS abc_1           , abc2.ach_returns                              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_returns                             , 0) != COALESCE(abc2.ach_returns                             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_sale_trans                           ' AS column_name, abc1.ach_sale_trans                           AS abc_1           , abc2.ach_sale_trans                           AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_sale_trans                          , 0) != COALESCE(abc2.ach_sale_trans                          , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_sale_vol                             ' AS column_name, abc1.ach_sale_vol                             AS abc_1           , abc2.ach_sale_vol                             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_sale_vol                            , 0) != COALESCE(abc2.ach_sale_vol                            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'ach_sale_volume_charge                   ' AS column_name, abc1.ach_sale_volume_charge                   AS abc_1           , abc2.ach_sale_volume_charge                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.ach_sale_volume_charge                  , 0) != COALESCE(abc2.ach_sale_volume_charge                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'apriva_monthly_charge                    ' AS column_name, abc1.apriva_monthly_charge                    AS abc_1           , abc2.apriva_monthly_charge                    AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.apriva_monthly_charge                   , 0) != COALESCE(abc2.apriva_monthly_charge                   , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'batch_files_processed                    ' AS column_name, abc1.batch_files_processed                    AS abc_1           , abc2.batch_files_processed                    AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.batch_files_processed                   , 0) != COALESCE(abc2.batch_files_processed                   , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'card_convenience_charge                  ' AS column_name, abc1.card_convenience_charge                  AS abc_1           , abc2.card_convenience_charge                  AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.card_convenience_charge                 , 0) != COALESCE(abc2.card_convenience_charge                 , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_auth_decline_trans                    ' AS column_name, abc1.cc_auth_decline_trans                    AS abc_1           , abc2.cc_auth_decline_trans                    AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_auth_decline_trans                   , 0) != COALESCE(abc2.cc_auth_decline_trans                   , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_auth_trans                            ' AS column_name, abc1.cc_auth_trans                            AS abc_1           , abc2.cc_auth_trans                            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_auth_trans                           , 0) != COALESCE(abc2.cc_auth_trans                           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_auth_vol                              ' AS column_name, abc1.cc_auth_vol                              AS abc_1           , abc2.cc_auth_vol                              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_auth_vol                             , 0) != COALESCE(abc2.cc_auth_vol                             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_batches                               ' AS column_name, abc1.cc_batches                               AS abc_1           , abc2.cc_batches                               AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_batches                              , 0) != COALESCE(abc2.cc_batches                              , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_capture_trans                         ' AS column_name, abc1.cc_capture_trans                         AS abc_1           , abc2.cc_capture_trans                         AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_capture_trans                        , 0) != COALESCE(abc2.cc_capture_trans                        , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_capture_vol                           ' AS column_name, abc1.cc_capture_vol                           AS abc_1           , abc2.cc_capture_vol                           AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_capture_vol                          , 0) != COALESCE(abc2.cc_capture_vol                          , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_credit_trans                          ' AS column_name, abc1.cc_credit_trans                          AS abc_1           , abc2.cc_credit_trans                          AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_credit_trans                         , 0) != COALESCE(abc2.cc_credit_trans                         , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_credit_vol                            ' AS column_name, abc1.cc_credit_vol                            AS abc_1           , abc2.cc_credit_vol                            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_credit_vol                           , 0) != COALESCE(abc2.cc_credit_vol                           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_keyed_trans                           ' AS column_name, abc1.cc_keyed_trans                           AS abc_1           , abc2.cc_keyed_trans                           AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_keyed_trans                          , 0) != COALESCE(abc2.cc_keyed_trans                          , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_keyed_vol                             ' AS column_name, abc1.cc_keyed_vol                             AS abc_1           , abc2.cc_keyed_vol                             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_keyed_vol                            , 0) != COALESCE(abc2.cc_keyed_vol                            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_ref_trans                             ' AS column_name, abc1.cc_ref_trans                             AS abc_1           , abc2.cc_ref_trans                             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_ref_trans                            , 0) != COALESCE(abc2.cc_ref_trans                            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_ref_vol                               ' AS column_name, abc1.cc_ref_vol                               AS abc_1           , abc2.cc_ref_vol                               AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_ref_vol                              , 0) != COALESCE(abc2.cc_ref_vol                              , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_sale_charge                           ' AS column_name, abc1.cc_sale_charge                           AS abc_1           , abc2.cc_sale_charge                           AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_sale_charge                          , 0) != COALESCE(abc2.cc_sale_charge                          , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_sale_decline_trans                    ' AS column_name, abc1.cc_sale_decline_trans                    AS abc_1           , abc2.cc_sale_decline_trans                    AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_sale_decline_trans                   , 0) != COALESCE(abc2.cc_sale_decline_trans                   , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_sale_trans                            ' AS column_name, abc1.cc_sale_trans                            AS abc_1           , abc2.cc_sale_trans                            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_sale_trans                           , 0) != COALESCE(abc2.cc_sale_trans                           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_sale_vol                              ' AS column_name, abc1.cc_sale_vol                              AS abc_1           , abc2.cc_sale_vol                              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_sale_vol                             , 0) != COALESCE(abc2.cc_sale_vol                             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_swiped_trans                          ' AS column_name, abc1.cc_swiped_trans                          AS abc_1           , abc2.cc_swiped_trans                          AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_swiped_trans                         , 0) != COALESCE(abc2.cc_swiped_trans                         , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'cc_swiped_vol                            ' AS column_name, abc1.cc_swiped_vol                            AS abc_1           , abc2.cc_swiped_vol                            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.cc_swiped_vol                           , 0) != COALESCE(abc2.cc_swiped_vol                           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'collection_method                        ' AS column_name, abc1.collection_method                        AS abc_1           , abc2.collection_method                        AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.collection_method                       , 0) != COALESCE(abc2.collection_method                       , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'combined_decline_trans                   ' AS column_name, abc1.combined_decline_trans                   AS abc_1           , abc2.combined_decline_trans                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.combined_decline_trans                  , 0) != COALESCE(abc2.combined_decline_trans                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'decryption_count                         ' AS column_name, abc1.decryption_count                         AS abc_1           , abc2.decryption_count                         AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.decryption_count                        , 0) != COALESCE(abc2.decryption_count                        , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'device_activated_count                   ' AS column_name, abc1.device_activated_count                   AS abc_1           , abc2.device_activated_count                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.device_activated_count                  , 0) != COALESCE(abc2.device_activated_count                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'device_activating_activated_count        ' AS column_name, abc1.device_activating_activated_count        AS abc_1           , abc2.device_activating_activated_count        AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.device_activating_activated_count       , 0) != COALESCE(abc2.device_activating_activated_count       , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'device_activating_count                  ' AS column_name, abc1.device_activating_count                  AS abc_1           , abc2.device_activating_count                  AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.device_activating_count                 , 0) != COALESCE(abc2.device_activating_count                 , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'device_other_activated_count             ' AS column_name, abc1.device_other_activated_count             AS abc_1           , abc2.device_other_activated_count             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.device_other_activated_count            , 0) != COALESCE(abc2.device_other_activated_count            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'device_other_count                       ' AS column_name, abc1.device_other_count                       AS abc_1           , abc2.device_other_count                       AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.device_other_count                      , 0) != COALESCE(abc2.device_other_count                      , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'device_stored_activated_count            ' AS column_name, abc1.device_stored_activated_count            AS abc_1           , abc2.device_stored_activated_count            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.device_stored_activated_count           , 0) != COALESCE(abc2.device_stored_activated_count           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'device_stored_count                      ' AS column_name, abc1.device_stored_count                      AS abc_1           , abc2.device_stored_count                      AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.device_stored_count                     , 0) != COALESCE(abc2.device_stored_count                     , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'file_transfer_monthly_charge             ' AS column_name, abc1.file_transfer_monthly_charge             AS abc_1           , abc2.file_transfer_monthly_charge             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.file_transfer_monthly_charge            , 0) != COALESCE(abc2.file_transfer_monthly_charge            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'group_charge                             ' AS column_name, abc1.group_charge                             AS abc_1           , abc2.group_charge                             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.group_charge                            , 0) != COALESCE(abc2.group_charge                            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'group_count                              ' AS column_name, abc1.group_count                              AS abc_1           , abc2.group_count                              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.group_count                             , 0) != COALESCE(abc2.group_count                             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'gw_monthly_charge                        ' AS column_name, abc1.gw_monthly_charge                        AS abc_1           , abc2.gw_monthly_charge                        AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.gw_monthly_charge                       , 0) != COALESCE(abc2.gw_monthly_charge                       , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'gw_per_auth_charge                       ' AS column_name, abc1.gw_per_auth_charge                       AS abc_1           , abc2.gw_per_auth_charge                       AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.gw_per_auth_charge                      , 0) != COALESCE(abc2.gw_per_auth_charge                      , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'gw_per_auth_decline_charge               ' AS column_name, abc1.gw_per_auth_decline_charge               AS abc_1           , abc2.gw_per_auth_decline_charge               AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.gw_per_auth_decline_charge              , 0) != COALESCE(abc2.gw_per_auth_decline_charge              , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'gw_per_credit_charge                     ' AS column_name, abc1.gw_per_credit_charge                     AS abc_1           , abc2.gw_per_credit_charge                     AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.gw_per_credit_charge                    , 0) != COALESCE(abc2.gw_per_credit_charge                    , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'gw_per_refund_charge                     ' AS column_name, abc1.gw_per_refund_charge                     AS abc_1           , abc2.gw_per_refund_charge                     AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.gw_per_refund_charge                    , 0) != COALESCE(abc2.gw_per_refund_charge                    , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'gw_per_token_charge                      ' AS column_name, abc1.gw_per_token_charge                      AS abc_1           , abc2.gw_per_token_charge                      AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.gw_per_token_charge                     , 0) != COALESCE(abc2.gw_per_token_charge                     , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'gw_reissued_ach_trans_charge             ' AS column_name, abc1.gw_reissued_ach_trans_charge             AS abc_1           , abc2.gw_reissued_ach_trans_charge             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.gw_reissued_ach_trans_charge            , 0) != COALESCE(abc2.gw_reissued_ach_trans_charge            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'gw_reissued_charge                       ' AS column_name, abc1.gw_reissued_charge                       AS abc_1           , abc2.gw_reissued_charge                       AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.gw_reissued_charge                      , 0) != COALESCE(abc2.gw_reissued_charge                      , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'hold_bill                                ' AS column_name, abc1.hold_bill                                AS abc_1           , abc2.hold_bill                                AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.hold_bill                               , 0) != COALESCE(abc2.hold_bill                               , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'misc_monthly_charge                      ' AS column_name, abc1.misc_monthly_charge                      AS abc_1           , abc2.misc_monthly_charge                      AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.misc_monthly_charge                     , 0) != COALESCE(abc2.misc_monthly_charge                     , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_active_device_trans                 ' AS column_name, abc1.p2pe_active_device_trans                 AS abc_1           , abc2.p2pe_active_device_trans                 AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_active_device_trans                , 0) != COALESCE(abc2.p2pe_active_device_trans                , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_auth_decline_trans                  ' AS column_name, abc1.p2pe_auth_decline_trans                  AS abc_1           , abc2.p2pe_auth_decline_trans                  AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_auth_decline_trans                 , 0) != COALESCE(abc2.p2pe_auth_decline_trans                 , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_auth_trans                          ' AS column_name, abc1.p2pe_auth_trans                          AS abc_1           , abc2.p2pe_auth_trans                          AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_auth_trans                         , 0) != COALESCE(abc2.p2pe_auth_trans                         , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_auth_vol                            ' AS column_name, abc1.p2pe_auth_vol                            AS abc_1           , abc2.p2pe_auth_vol                            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_auth_vol                           , 0) != COALESCE(abc2.p2pe_auth_vol                           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_capture_trans                       ' AS column_name, abc1.p2pe_capture_trans                       AS abc_1           , abc2.p2pe_capture_trans                       AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_capture_trans                      , 0) != COALESCE(abc2.p2pe_capture_trans                      , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_capture_vol                         ' AS column_name, abc1.p2pe_capture_vol                         AS abc_1           , abc2.p2pe_capture_vol                         AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_capture_vol                        , 0) != COALESCE(abc2.p2pe_capture_vol                        , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_credit_trans                        ' AS column_name, abc1.p2pe_credit_trans                        AS abc_1           , abc2.p2pe_credit_trans                        AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_credit_trans                       , 0) != COALESCE(abc2.p2pe_credit_trans                       , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_credit_vol                          ' AS column_name, abc1.p2pe_credit_vol                          AS abc_1           , abc2.p2pe_credit_vol                          AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_credit_vol                         , 0) != COALESCE(abc2.p2pe_credit_vol                         , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_declined_trans                      ' AS column_name, abc1.p2pe_declined_trans                      AS abc_1           , abc2.p2pe_declined_trans                      AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_declined_trans                     , 0) != COALESCE(abc2.p2pe_declined_trans                     , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_device_activated_charge             ' AS column_name, abc1.p2pe_device_activated_charge             AS abc_1           , abc2.p2pe_device_activated_charge             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_device_activated_charge            , 0) != COALESCE(abc2.p2pe_device_activated_charge            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_device_activating_charge            ' AS column_name, abc1.p2pe_device_activating_charge            AS abc_1           , abc2.p2pe_device_activating_charge            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_device_activating_charge           , 0) != COALESCE(abc2.p2pe_device_activating_charge           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_device_stored_charge                ' AS column_name, abc1.p2pe_device_stored_charge                AS abc_1           , abc2.p2pe_device_stored_charge                AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_device_stored_charge               , 0) != COALESCE(abc2.p2pe_device_stored_charge               , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_encryption_charge                   ' AS column_name, abc1.p2pe_encryption_charge                   AS abc_1           , abc2.p2pe_encryption_charge                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_encryption_charge                  , 0) != COALESCE(abc2.p2pe_encryption_charge                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_inactive_device_trans               ' AS column_name, abc1.p2pe_inactive_device_trans               AS abc_1           , abc2.p2pe_inactive_device_trans               AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_inactive_device_trans              , 0) != COALESCE(abc2.p2pe_inactive_device_trans              , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_refund_trans                        ' AS column_name, abc1.p2pe_refund_trans                        AS abc_1           , abc2.p2pe_refund_trans                        AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_refund_trans                       , 0) != COALESCE(abc2.p2pe_refund_trans                       , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_refund_vol                          ' AS column_name, abc1.p2pe_refund_vol                          AS abc_1           , abc2.p2pe_refund_vol                          AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_refund_vol                         , 0) != COALESCE(abc2.p2pe_refund_vol                         , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_sale_decline_trans                  ' AS column_name, abc1.p2pe_sale_decline_trans                  AS abc_1           , abc2.p2pe_sale_decline_trans                  AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_sale_decline_trans                 , 0) != COALESCE(abc2.p2pe_sale_decline_trans                 , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_sale_trans                          ' AS column_name, abc1.p2pe_sale_trans                          AS abc_1           , abc2.p2pe_sale_trans                          AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_sale_trans                         , 0) != COALESCE(abc2.p2pe_sale_trans                         , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_sale_vol                            ' AS column_name, abc1.p2pe_sale_vol                            AS abc_1           , abc2.p2pe_sale_vol                            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_sale_vol                           , 0) != COALESCE(abc2.p2pe_sale_vol                           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_tokens_stored                       ' AS column_name, abc1.p2pe_tokens_stored                       AS abc_1           , abc2.p2pe_tokens_stored                       AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_tokens_stored                      , 0) != COALESCE(abc2.p2pe_tokens_stored                      , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_token_charge                        ' AS column_name, abc1.p2pe_token_charge                        AS abc_1           , abc2.p2pe_token_charge                        AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_token_charge                       , 0) != COALESCE(abc2.p2pe_token_charge                       , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_token_flat_charge                   ' AS column_name, abc1.p2pe_token_flat_charge                   AS abc_1           , abc2.p2pe_token_flat_charge                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_token_flat_charge                  , 0) != COALESCE(abc2.p2pe_token_flat_charge                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'p2pe_token_flat_monthly_charge           ' AS column_name, abc1.p2pe_token_flat_monthly_charge           AS abc_1           , abc2.p2pe_token_flat_monthly_charge           AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.p2pe_token_flat_monthly_charge          , 0) != COALESCE(abc2.p2pe_token_flat_monthly_charge          , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pci_compliance_fee                       ' AS column_name, abc1.pci_compliance_fee                       AS abc_1           , abc2.pci_compliance_fee                       AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pci_compliance_fee                      , 0) != COALESCE(abc2.pci_compliance_fee                      , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pci_monthly_charge                       ' AS column_name, abc1.pci_monthly_charge                       AS abc_1           , abc2.pci_monthly_charge                       AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pci_monthly_charge                      , 0) != COALESCE(abc2.pci_monthly_charge                      , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pci_non_compliance_charge                ' AS column_name, abc1.pci_non_compliance_charge                AS abc_1           , abc2.pci_non_compliance_charge                AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pci_non_compliance_charge               , 0) != COALESCE(abc2.pci_non_compliance_charge               , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pci_non_compliance_fee                   ' AS column_name, abc1.pci_non_compliance_fee                   AS abc_1           , abc2.pci_non_compliance_fee                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pci_non_compliance_fee                  , 0) != COALESCE(abc2.pci_non_compliance_fee                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pci_scans_monthly_charge                 ' AS column_name, abc1.pci_scans_monthly_charge                 AS abc_1           , abc2.pci_scans_monthly_charge                 AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pci_scans_monthly_charge                , 0) != COALESCE(abc2.pci_scans_monthly_charge                , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pc_account_updater_monthly_charge        ' AS column_name, abc1.pc_account_updater_monthly_charge        AS abc_1           , abc2.pc_account_updater_monthly_charge        AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pc_account_updater_monthly_charge       , 0) != COALESCE(abc2.pc_account_updater_monthly_charge       , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_ach_credit_fee                   ' AS column_name, abc1.pricing_ach_credit_fee                   AS abc_1           , abc2.pricing_ach_credit_fee                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_ach_credit_fee                  , 0) != COALESCE(abc2.pricing_ach_credit_fee                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_ach_discount_rate                ' AS column_name, abc1.pricing_ach_discount_rate                AS abc_1           , abc2.pricing_ach_discount_rate                AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_ach_discount_rate               , 0) != COALESCE(abc2.pricing_ach_discount_rate               , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_ach_monthly_fee                  ' AS column_name, abc1.pricing_ach_monthly_fee                  AS abc_1           , abc2.pricing_ach_monthly_fee                  AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_ach_monthly_fee                 , 0) != COALESCE(abc2.pricing_ach_monthly_fee                 , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_ach_noc_fee                      ' AS column_name, abc1.pricing_ach_noc_fee                      AS abc_1           , abc2.pricing_ach_noc_fee                      AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_ach_noc_fee                     , 0) != COALESCE(abc2.pricing_ach_noc_fee                     , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_ach_per_gw_trans_fee             ' AS column_name, abc1.pricing_ach_per_gw_trans_fee             AS abc_1           , abc2.pricing_ach_per_gw_trans_fee             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_ach_per_gw_trans_fee            , 0) != COALESCE(abc2.pricing_ach_per_gw_trans_fee            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_ach_return_error_fee             ' AS column_name, abc1.pricing_ach_return_error_fee             AS abc_1           , abc2.pricing_ach_return_error_fee             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_ach_return_error_fee            , 0) != COALESCE(abc2.pricing_ach_return_error_fee            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_ach_transaction_fee              ' AS column_name, abc1.pricing_ach_transaction_fee              AS abc_1           , abc2.pricing_ach_transaction_fee              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_ach_transaction_fee             , 0) != COALESCE(abc2.pricing_ach_transaction_fee             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_bluefin_gateway_discount_rate    ' AS column_name, abc1.pricing_bluefin_gateway_discount_rate    AS abc_1           , abc2.pricing_bluefin_gateway_discount_rate    AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_bluefin_gateway_discount_rate   , 0) != COALESCE(abc2.pricing_bluefin_gateway_discount_rate   , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_file_transfer_monthly_fee        ' AS column_name, abc1.pricing_file_transfer_monthly_fee        AS abc_1           , abc2.pricing_file_transfer_monthly_fee        AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_file_transfer_monthly_fee       , 0) != COALESCE(abc2.pricing_file_transfer_monthly_fee       , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_gateway_monthly_fee              ' AS column_name, abc1.pricing_gateway_monthly_fee              AS abc_1           , abc2.pricing_gateway_monthly_fee              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_gateway_monthly_fee             , 0) != COALESCE(abc2.pricing_gateway_monthly_fee             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_group_tag_fee                    ' AS column_name, abc1.pricing_group_tag_fee                    AS abc_1           , abc2.pricing_group_tag_fee                    AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_group_tag_fee                   , 0) != COALESCE(abc2.pricing_group_tag_fee                   , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_gw_per_auth_decline_fee          ' AS column_name, abc1.pricing_gw_per_auth_decline_fee          AS abc_1           , abc2.pricing_gw_per_auth_decline_fee          AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_gw_per_auth_decline_fee         , 0) != COALESCE(abc2.pricing_gw_per_auth_decline_fee         , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_gw_per_credit_fee                ' AS column_name, abc1.pricing_gw_per_credit_fee                AS abc_1           , abc2.pricing_gw_per_credit_fee                AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_gw_per_credit_fee               , 0) != COALESCE(abc2.pricing_gw_per_credit_fee               , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_gw_per_refund_fee                ' AS column_name, abc1.pricing_gw_per_refund_fee                AS abc_1           , abc2.pricing_gw_per_refund_fee                AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_gw_per_refund_fee               , 0) != COALESCE(abc2.pricing_gw_per_refund_fee               , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_gw_per_sale_fee                  ' AS column_name, abc1.pricing_gw_per_sale_fee                  AS abc_1           , abc2.pricing_gw_per_sale_fee                  AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_gw_per_sale_fee                 , 0) != COALESCE(abc2.pricing_gw_per_sale_fee                 , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_gw_per_token_fee                 ' AS column_name, abc1.pricing_gw_per_token_fee                 AS abc_1           , abc2.pricing_gw_per_token_fee                 AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_gw_per_token_fee                , 0) != COALESCE(abc2.pricing_gw_per_token_fee                , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_gw_reissued_fee                  ' AS column_name, abc1.pricing_gw_reissued_fee                  AS abc_1           , abc2.pricing_gw_reissued_fee                  AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_gw_reissued_fee                 , 0) != COALESCE(abc2.pricing_gw_reissued_fee                 , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_misc_monthly_fee                 ' AS column_name, abc1.pricing_misc_monthly_fee                 AS abc_1           , abc2.pricing_misc_monthly_fee                 AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_misc_monthly_fee                , 0) != COALESCE(abc2.pricing_misc_monthly_fee                , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_one_time_key_injection_fee       ' AS column_name, abc1.pricing_one_time_key_injection_fee       AS abc_1           , abc2.pricing_one_time_key_injection_fee       AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_one_time_key_injection_fee      , 0) != COALESCE(abc2.pricing_one_time_key_injection_fee      , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_p2pe_device_activated_fee        ' AS column_name, abc1.pricing_p2pe_device_activated_fee        AS abc_1           , abc2.pricing_p2pe_device_activated_fee        AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_p2pe_device_activated_fee       , 0) != COALESCE(abc2.pricing_p2pe_device_activated_fee       , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_p2pe_device_activating_fee       ' AS column_name, abc1.pricing_p2pe_device_activating_fee       AS abc_1           , abc2.pricing_p2pe_device_activating_fee       AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_p2pe_device_activating_fee      , 0) != COALESCE(abc2.pricing_p2pe_device_activating_fee      , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_p2pe_device_stored_fee           ' AS column_name, abc1.pricing_p2pe_device_stored_fee           AS abc_1           , abc2.pricing_p2pe_device_stored_fee           AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_p2pe_device_stored_fee          , 0) != COALESCE(abc2.pricing_p2pe_device_stored_fee          , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_p2pe_encryption_fee              ' AS column_name, abc1.pricing_p2pe_encryption_fee              AS abc_1           , abc2.pricing_p2pe_encryption_fee              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_p2pe_encryption_fee             , 0) != COALESCE(abc2.pricing_p2pe_encryption_fee             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_p2pe_monthly_flat_fee            ' AS column_name, abc1.pricing_p2pe_monthly_flat_fee            AS abc_1           , abc2.pricing_p2pe_monthly_flat_fee            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_p2pe_monthly_flat_fee           , 0) != COALESCE(abc2.pricing_p2pe_monthly_flat_fee           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_p2pe_tokenization_fee            ' AS column_name, abc1.pricing_p2pe_tokenization_fee            AS abc_1           , abc2.pricing_p2pe_tokenization_fee            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_p2pe_tokenization_fee           , 0) != COALESCE(abc2.pricing_p2pe_tokenization_fee           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_pci_scans_monthly_fee            ' AS column_name, abc1.pricing_pci_scans_monthly_fee            AS abc_1           , abc2.pricing_pci_scans_monthly_fee            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_pci_scans_monthly_fee           , 0) != COALESCE(abc2.pricing_pci_scans_monthly_fee           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_pc_acct_updater_fee              ' AS column_name, abc1.pricing_pc_acct_updater_fee              AS abc_1           , abc2.pricing_pc_acct_updater_fee              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_pc_acct_updater_fee             , 0) != COALESCE(abc2.pricing_pc_acct_updater_fee             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_per_transaction_fee              ' AS column_name, abc1.pricing_per_transaction_fee              AS abc_1           , abc2.pricing_per_transaction_fee              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_per_transaction_fee             , 0) != COALESCE(abc2.pricing_per_transaction_fee             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_shieldconex_fields_fee           ' AS column_name, abc1.pricing_shieldconex_fields_fee           AS abc_1           , abc2.pricing_shieldconex_fields_fee           AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_shieldconex_fields_fee          , 0) != COALESCE(abc2.pricing_shieldconex_fields_fee          , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_shieldconex_monthly_fee          ' AS column_name, abc1.pricing_shieldconex_monthly_fee          AS abc_1           , abc2.pricing_shieldconex_monthly_fee          AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_shieldconex_monthly_fee         , 0) != COALESCE(abc2.pricing_shieldconex_monthly_fee         , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_shieldconex_monthly_minimum_fee  ' AS column_name, abc1.pricing_shieldconex_monthly_minimum_fee  AS abc_1           , abc2.pricing_shieldconex_monthly_minimum_fee  AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_shieldconex_monthly_minimum_fee , 0) != COALESCE(abc2.pricing_shieldconex_monthly_minimum_fee , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'pricing_shieldconex_transaction_fee      ' AS column_name, abc1.pricing_shieldconex_transaction_fee      AS abc_1           , abc2.pricing_shieldconex_transaction_fee      AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.pricing_shieldconex_transaction_fee     , 0) != COALESCE(abc2.pricing_shieldconex_transaction_fee     , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'reissued_ach_transactions                ' AS column_name, abc1.reissued_ach_transactions                AS abc_1           , abc2.reissued_ach_transactions                AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.reissued_ach_transactions               , 0) != COALESCE(abc2.reissued_ach_transactions               , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'reissued_cc_transactions                 ' AS column_name, abc1.reissued_cc_transactions                 AS abc_1           , abc2.reissued_cc_transactions                 AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.reissued_cc_transactions                , 0) != COALESCE(abc2.reissued_cc_transactions                , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'shieldconex_fields_charge                ' AS column_name, abc1.shieldconex_fields_charge                AS abc_1           , abc2.shieldconex_fields_charge                AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.shieldconex_fields_charge               , 0) != COALESCE(abc2.shieldconex_fields_charge               , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'shieldconex_monthly_charge               ' AS column_name, abc1.shieldconex_monthly_charge               AS abc_1           , abc2.shieldconex_monthly_charge               AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.shieldconex_monthly_charge              , 0) != COALESCE(abc2.shieldconex_monthly_charge              , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'shieldconex_monthly_minimum_charge       ' AS column_name, abc1.shieldconex_monthly_minimum_charge       AS abc_1           , abc2.shieldconex_monthly_minimum_charge       AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.shieldconex_monthly_minimum_charge      , 0) != COALESCE(abc2.shieldconex_monthly_minimum_charge      , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'shieldconex_transaction_charge           ' AS column_name, abc1.shieldconex_transaction_charge           AS abc_1           , abc2.shieldconex_transaction_charge           AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.shieldconex_transaction_charge          , 0) != COALESCE(abc2.shieldconex_transaction_charge          , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'tokens_stored                            ' AS column_name, abc1.tokens_stored                            AS abc_1           , abc2.tokens_stored                            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.tokens_stored                           , 0) != COALESCE(abc2.tokens_stored                           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'total_bad_detokenized                    ' AS column_name, abc1.total_bad_detokenized                    AS abc_1           , abc2.total_bad_detokenized                    AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.total_bad_detokenized                   , 0) != COALESCE(abc2.total_bad_detokenized                   , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'total_bad_detokenized_fields             ' AS column_name, abc1.total_bad_detokenized_fields             AS abc_1           , abc2.total_bad_detokenized_fields             AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.total_bad_detokenized_fields            , 0) != COALESCE(abc2.total_bad_detokenized_fields            , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'total_bad_tokenized                      ' AS column_name, abc1.total_bad_tokenized                      AS abc_1           , abc2.total_bad_tokenized                      AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.total_bad_tokenized                     , 0) != COALESCE(abc2.total_bad_tokenized                     , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'total_bad_tokenized_fields               ' AS column_name, abc1.total_bad_tokenized_fields               AS abc_1           , abc2.total_bad_tokenized_fields               AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.total_bad_tokenized_fields              , 0) != COALESCE(abc2.total_bad_tokenized_fields              , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'total_good_detokenized                   ' AS column_name, abc1.total_good_detokenized                   AS abc_1           , abc2.total_good_detokenized                   AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.total_good_detokenized                  , 0) != COALESCE(abc2.total_good_detokenized                  , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'total_good_detokenized_fields            ' AS column_name, abc1.total_good_detokenized_fields            AS abc_1           , abc2.total_good_detokenized_fields            AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.total_good_detokenized_fields           , 0) != COALESCE(abc2.total_good_detokenized_fields           , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'total_good_tokenized                     ' AS column_name, abc1.total_good_tokenized                     AS abc_1           , abc2.total_good_tokenized                     AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.total_good_tokenized                    , 0) != COALESCE(abc2.total_good_tokenized                    , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'total_good_tokenized_fields              ' AS column_name, abc1.total_good_tokenized_fields              AS abc_1           , abc2.total_good_tokenized_fields              AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.total_good_tokenized_fields             , 0) != COALESCE(abc2.total_good_tokenized_fields             , 0)  
  UNION SELECT abc1.cardconex_acct_id, 'user_count                               ' AS column_name, abc1.user_count                               AS abc_1           , abc2.user_count                               AS abc_2            FROM auto_billing_dw.f_auto_billing_complete_shieldconex  AS abc1 LEFT JOIN auto_billing_dw.f_auto_billing_complete_2       AS abc2 ON abc1.cardconex_acct_id = abc2.account_id   WHERE COALESCE(abc1.user_count                              , 0) != COALESCE(abc2.user_count                              , 0)  
  ;
  
  UPDATE test.tmp_abc_err
     SET column_name = TRIM(column_name)
   WHERE TRUE  
  ;
  
  ALTER TABLE test.tmp_abc_err
    ADD COLUMN is_fee BOOLEAN AFTER cardconex_acct_id 
   ,ADD COLUMN is_charge BOOLEAN AFTER is_fee 
   ,ADD COLUMN is_count BOOLEAN AFTER is_charge
   ,ADD COLUMN is_trans BOOLEAN AFTER is_count
   ,ADD COLUMN is_vol BOOLEAN AFTER is_trans
   ,ADD COLUMN is_other BOOLEAN AFTER is_vol
  ;
  
--   ALTER TABLE test.tmp_abc_err
--     ADD COLUMN account_name VARCHAR(128) AFTER cardconex_account_id;

  UPDATE test.tmp_abc_err
     SET is_fee = IF(column_name LIKE '%fee', 1, 0)
        ,is_charge = IF(column_name LIKE '%charge', 1, 0)
        ,is_count = IF(column_name LIKE '%count', 1, 0)
        ,is_trans = IF(column_name LIKE '%trans%', 1, 0)
        ,is_vol = IF(column_name LIKE '%vol', 1, 0)
   WHERE TRUE 
  ;
  
  UPDATE test.tmp_abc_err 
     SET is_other = 1 
   WHERE is_fee = 0 AND is_charge = 0 AND is_count = 0 AND is_trans = 0 AND is_vol = 0
  ;
  
--   UPDATE test.tmp_abc_err       t1
--     JOIN sales_force.account    t2 
--       ON t1.cardconex_acct_id = t2.id 
--      SET t1.account_name = t2.name 
--    WHERE TRUE 
--   ;
    
  SELECT column_name, COUNT(*) AS num_rows
    FROM test.tmp_abc_err 
   GROUP BY 1
  ;
...
-- Dump completed on 2021-02-03 14:28:55
